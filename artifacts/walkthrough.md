...
*   **Root Cause**: The User model had `unique: true` on the `email` field while also specifying `userSchema.index({ email: 1 }, { unique: true });`. This led to a duplicate schema index warning on startup.
...
- **Production Build**: Vite build compiles 1425 modules successfully in `21.71s`.
